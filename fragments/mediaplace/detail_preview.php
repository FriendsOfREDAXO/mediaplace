...
<?php if ($info['is_image']): ?>
    ...
    <div class="mp3-detail-preview">
        <button type="button" class="mp3-lightbox-open-btn" data-lightbox-src="<?= rex_escape($src) ?>" data-lightbox-caption="<?= rex_escape($info['title'] !== '' ? $info['title'] : $filename) ?>" title="Lightbox öffnen"><i class="fa-solid fa-up-right-and-down-left-from-center"></i></button>
        <?php if (!empty($info['focuspoint_available'])): ?>
            <button type="button" class="mp3-focuspoint-edit-btn" data-focuspoint-file="<?= rex_escape($filename) ?>" title="Fokuspunkt bearbeiten"><i class="fa-solid fa-crosshairs"></i></button>
        <?php endif; ?>
...
<?php elseif (DetailPanelFormatter::isPdfFilename($filename)): ?>
    ...
    <div class="mp3-detail-preview mp3-detail-preview-icon">
        <a class="mp3-pdf-open-btn" href="<?= rex_escape($pdfSrc) ?>" target="_blank" rel="noopener" title="PDF in neuem Tab öffnen"><i class="fa-solid fa-up-right-from-square"></i></a>
        <i class="<?= rex_escape(DetailPanelFormatter::fileIconClass($filename)) ?>"></i>
    </div>
<?php else: ?>
    <div class="mp3-detail-preview mp3-detail-preview-icon">

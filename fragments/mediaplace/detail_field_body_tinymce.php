...
if (!empty($field['translatable'])) {
    ...
    foreach ($rows as $clang) {
        ...
        <div class="mp3-tiny-canvas-row">
            ...
            <button type="button" class="mp3-tiny-canvas-open" data-canvas-field="<?= rex_escape($key) ?>" data-canvas-clang="<?= rex_escape((string) $clang['id']) ?>" data-canvas-label="<?= rex_escape($label . ' (' . $badge . ')') ?>"><i class="fa-solid fa-pen-to-square"></i> <?= rex_escape($this->i18n('mediaplace_edit')) ?></button>
        </div>
        <?php
    }
    return;
...
<div class="mp3-tiny-canvas-row">
    ...
    <button type="button" class="mp3-tiny-canvas-open" data-canvas-field="<?= rex_escape($key) ?>" data-canvas-label="<?= rex_escape($label) ?>"><i class="fa-solid fa-pen-to-square"></i> <?= rex_escape($this->i18n('mediaplace_edit')) ?></button>
</div>

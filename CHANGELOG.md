...
- Zahnrad-Menü-Erweiterungspunkt: andere Addons können über `MP3.registerAdminMenuItem(id, {label, icon, onClick})` einen eigenen Eintrag ins Verwaltungsmenü einhängen, der JS-Code innerhalb des laufenden Overlays ausführt (anders als die bisherige Liste klassischer Unterseiten, die immer im Popup-Fenster öffnet). Erster Nutzer: das `mediaplace_a11y`-Addon ("AI Bulk Management").

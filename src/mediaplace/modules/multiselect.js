...
export function updateCollectionDragSelectionUI() {
    ...
    qsa('.mp3-masonry-card', grid).forEach(function (m) {
        ...
        var isSel = !!collectionDragSelected[fn];
        m.classList.toggle('mp3-masonry-card-multi', isSel);
        if (batchSelectMode) {
            var chk = qs('.mp3-masonry-check i', m);
            if (chk) chk.className = 'fa-solid ' + (isSel ? 'fa-square-check' : 'fa-square');
        }
    });
...
// Toolbar-Button .mp3-select-mode-toggle): schaltet die Checkbox-Overlays
...
export function setBatchSelectMode(enabled) {
    ...
    var btn = qs('.mp3-select-mode-toggle', overlay);
    if (btn) btn.classList.toggle('mp3-select-mode-active', isEnabled);
    ctx.refreshDisplay();
}
